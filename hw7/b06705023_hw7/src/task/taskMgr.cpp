...
TaskMgr::add(size_t nMachines)
{
   ...
   for(int i=0;i<nMachines;i++)
   {
      ...
      if(this->_taskHash.insert(newN))
      {
         cout << "Task node inserted: (" << newN.getName() << ", " << newN.getLoad() << ")" << endl;
         this->_taskHeap.insert(newN);
      }
      else
      {
         i--;
...
TaskMgr::assign(size_t l)
{
   ...
   if(this->size()==0)
      return false;
   TaskNode nm(this->_taskHeap.min().getName(),this->_taskHeap.min().getLoad()+l);
   this->_taskHash.update(nm);
   this->_taskHeap.delMin();
   this->_taskHeap.insert(nm);
